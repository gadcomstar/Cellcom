...
public partial class admin_EditGeneralPage : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        CatFormView.ReturnURL = "ManageGeneralPage.aspx?type=2&cat=" + Request.QueryString["cat"] + "&sub=" + Request.QueryString["sub"] + "";
        backLink.NavigateUrl = "ManageGeneralPage.aspx?type=2&cat=" + Request.QueryString["cat"] + "&sub=" + Request.QueryString["sub"] + "";
        query = "IsChild=false"; 
        ...
        if (!String.IsNullOrEmpty(Request.QueryString["Parent"]) && int.TryParse(Request.QueryString["Parent"] , out parentID))
        {
            ...
            CatFormView.ReturnURL = "ManageGeneralPage.aspx?type=2&cat=" + Request.QueryString["cat"] + "&sub=" + Request.QueryString["sub"] + "&Parent=" + Request.QueryString["Parent"];
            backLink.NavigateUrl = "ManageGeneralPage.aspx?type=2&cat=" + Request.QueryString["cat"] + "&sub=" + Request.QueryString["sub"] + "&Parent=" + Request.QueryString["Parent"];
     
            IsPageOnMainMenu = "false";
            IsChild = "true";
            using (MySqlConnection conn = new MySqlConnection(ConnStr))
            {
                string sql = "Select * From Pages Where pageID="+parentID;
        ...
        if (Request.QueryString["id"] != null)
		{
            using (MySqlConnection conn = new MySqlConnection(ConnStr))
            {
                string sql = "Select `PageName` From pages Where PageID=" + Request.QueryString["id"];
                MySqlCommand cmd = new MySqlCommand(sql, conn);
                conn.Open();
                MySqlDataReader dr = cmd.ExecuteReader();
                if (dr.Read())
        ...
        else
		{
            ...
            using (MySqlConnection con = new MySqlConnection(siteDefaults.ConnStr))
            {
                ...
                cmd.Connection = con;
                cmd.CommandText = "SELECT Max(PageOrder) as MaxOrder FROM pages where " + query;
    ...
    {    
        ...
        using (MySqlConnection conn = new MySqlConnection(ConnStr))
        {
            string sql = "Select * From pages Where PageID="+id ;
            conn.Open();
            MySqlCommand cmd = new MySqlCommand(sql,conn);
            MySqlDataReader dr = cmd.ExecuteReader();
            if (dr.Read())
            ...
            if (UpdateUrl)
            {
                cmd.CommandText = "Update pages Set PageURL=\"" + (PageURL.Contains("?IntPage") ? "" : PageURL) + "?IntPage=" + id + "\",PageIsDel=true  Where PageID=" + id;
                cmd.ExecuteNonQuery();
